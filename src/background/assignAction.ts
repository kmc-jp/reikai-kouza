...
export const assignAction = async (payload: any) => {
  const assignedGroup: number = (await executeQuery(`SELECT assignment_group FROM ${projectConstants.mysql.tableName} WHERE id = ?`,
  [
    payload["user"]["id"],
  ]))[0]["assignment_group"];
  ...
  switch (payload["actions"][0]["selected_option"]["value"]) {
    case projectConstants.interactivity.values.assign.OK:
      await executeQuery(`UPDATE ${projectConstants.mysql.tableName} SET assigned_date = ?, announcement_status = ? WHERE id = ?`,
      [
        assignedGroup,
        10,
        payload["user"]["id"],
