...
const publicAnnounce = async () => {
  ...
  const assignmentInfo = await executeQuery<tableStructure__ID & tableStructure__assignedDate>(`SELECT ${tableItemName.id}, ${tableItemName.assignedDate} FROM ${projectConstants.mysql.tableName} WHERE (${tableItemName.assignedDate} >= ? AND ${tableItemName.assignedDate} <= ?)`,
  [
    today__dbFormat,
    today_oneWeekAfter__dbFormat,
  ]);
